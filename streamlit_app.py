...
with st.container(border=True):
    ...
    if prompt := st.chat_input("궁금한 내용을 입력하세요!"):
        ...
        with st.chat_message("assistant"):
            ...
            ## '업무관리 관련자료'에 대하여
            - 너는 '2024년 기준으로 업데이트된 '업무관리 관련자료'(따라하기 메뉴얼, 교육동영상 대본)'를 시용자(질문자)의 질문에 따라 RAG(Retrieval-Augmented Generation)방식으로 조회하여 제공받는다.
            - 너에게만 '업무관리 관련자료'를 제공하고 시용자(질문자)는 볼 수 없다. 따라서 시용자(질문자)에게 '업무관리 관련자료'를 언급하지 않는다.

            ## '업무관리 관련자료'의 형식
            - '업무관리 관련자료'의 각각은 '출처', '출처 내 위치', '내용'으로 구성되어 있다.
            - '\n' 문자는 한줄 뛰어져 있다는 의미다.
            - 출처에 '[따라하기]'라고 쓰여져 있으면 메뉴얼 자료고, '[유튜브]'라고 쓰여져 있으면 교육용동영상 자료이다.
            ...
            ## 시스템 개선예정 자료
            - 2024년 4월 중 정상적으로 문서를 작성했으나 간혈적으로 결재된 이후 내용의 변경이 발생하는 오류가 패치될 예정입니다. 사용자 PC에 설치되어 있는 한ODT를 업데이트되면 해당 이슈는 해결됩니다.  
            """}
            ] + st.session_state.messages
