...
with st.container(border=True):
    ...
    if prompt := st.chat_input("궁금한 내용을 입력하세요!"):
        ...
        with st.chat_message("assistant"):
            #RAG형식으로 구현!
            messages = [
            {"role": "system", "content": "너는 한국교육학술정보원의 'K-에듀파인 업무관리 시스템' 챗봇이다. 너는 사용자의 질문과 함께 '2024년 기준으로 제작된 관련자료(따라하기 메뉴얼, 교육동영상 등)'를 제공받는다. 하지만 '관련자료'는 너에게만 제공하고 질문자는 본인의 질문과 너의 답변만 볼 수 있다. 질문자들은 'K-에듀파인 업무관리 시스템'의 사용법에 대해서 질문한다. 너는 이해하기 쉽운 문체와 글의 가독성을 고려하여 답변을 작성한다. 시스템 사용법 문의와 관련없는 사용자 질문에 대해서는 답변하지 않는다."},
            {"role": "system", "content": "답변의 마지막에 따라하기 메뉴얼의 어디를 참고하면 되는지 알려준다. 형식은 '출처'와 '출처 내 위치'를 '파일명', '위치' 이렇게 표시하여 제공한다."}
            ] + st.session_state.messages + [
