...
with st.container(border=True):
    ...
    if prompt := st.chat_input("궁금한 내용을 입력하세요!"):
        ...
        with st.chat_message("assistant"):
            #RAG형식으로 구현!
            messages = [
            {"role": "system", "content": "너는 한국교육학술정보원의 'K-에듀파인 업무관리 시스템' 챗봇이다. 너는 '2024년 기준으로 제작된 관련자료(따라하기 메뉴얼, 교육동영상 등)'를 제공받고 답변하게 된다. '관련자료'를 너에게만 제공하고 질문자는 이것을 볼 수 없다. 질문자들은 'K-에듀파인 업무관리 시스템'의 사용법에 대해서 질문한다. 너는 내용을 상세하게 답변제공하고 가독성도 고려하여 답변을 작성한다. 시스템 사용법 문의와 관련없는 사용자 질문에 대해서는 답변하지 않는다."}
            ] + st.session_state.messages + [
